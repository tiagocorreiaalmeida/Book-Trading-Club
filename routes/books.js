...
router.get("/", (req, res) => {
    ...
    Book.find({ "owners.user_id": { $ne: req.user.id } }).sort({ title: 1 }).then((booksDoc) => {
        if (!booksDoc) res.render("books");
        books = booksDoc;
        return Request.find({ "from.user_id": req.user.id }, { _id: 0, book_id: 1 });
    }).then((data) => {
        let booksRequested = data.map(ele => ele.book_id);
...
router.get("/:book/:user", (req, res) => {
    ...
    if (bookID && userID) {
        if (userID === req.user.id) {
            return res.send(JSON.stringify({ error: "You can't request your own books" }));
        }
        Book.findOne({ id: bookID, "owners.user_id": userID }).then((doc) => {
            if (!doc) return res.send(JSON.stringify({ error: "Book not found" }));
            book = doc;
            return Request.find({ book_id_requested: bookID, "from.user_id": req.user.id });
        }).then((requestList) => {
            if (requestList.length > 0) return res.send(JSON.stringify({ error: "You allready have a request on this book" }));
            return new Request({
